    ...
    >
      ...
      <div className="grid sm:grid-cols-3 grid-cols-1 lg:gap-x-16 md:gap-x-6 lg:gap-x-20 pt-10">
        {category.map((item: any, index: number) => {
          ...
          return (
            <Link href={href} key={item.description + index} className="pb-20">
              <CardCategory item={item} active={active} />
            </Link>
          );
        })}
      </div>
...
const CardCategory = ({ item, active }: any) => {
  const { slug } = item;
  ...
  return (
    ...
    >
      ...
      <div className="pt-10 flex flex-col items-center justify-center w-full">
        <p className="text-3xl text-center hover:text-amber-500 duration-300 cursor-pointer text-title w-full truncate-2">
          {item.title}
        </p>
